namespace Dreamteck.Splines
{
	...
	public class SampleCollection
	{
		...
		//legacy properties
		[HideInInspector] public double clipFrom = 0.0, clipTo = 1.0;
		[HideInInspector] public bool loopSamples = false;
		...
		public double TravelWithOffset(double start, float distance, Spline.Direction direction, Vector3 offset, out float moved, double clipFrom = 0.0, double clipTo = 1.0)
		{
			...
			while (moved < distance)
			{
				Vector3 newPos = samples[sampleIndex].position + samples[sampleIndex].up * offset.y * samples[sampleIndex].size + samples[sampleIndex].right * offset.x * samples[sampleIndex].size +
				...
				else
				{
					if (sampleIndex == 0)
					{
						if (samplesAreLooped)
						{
							...
							lastPos = samples[lastIndex].position + samples[lastIndex].up * offset.y * samples[lastIndex].size + samples[lastIndex].right * offset.x * samples[lastIndex].size +
