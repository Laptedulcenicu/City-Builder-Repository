    ...
    public class TouchInputController : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            if (isInputOnLockedArea == false)
            {
                ...
                if (isPinching == false)
                {
                    if (wasPinchingLastFrame == false)
                    {
                        if (wasFingerDownLastFrame == true && TouchWrapper.IsFingerDown)
                        {
                            ...
                            {
                                float dragDistance =
                                    GetRelativeDragDistance(TouchWrapper.Touch0.Position, dragStartPos);
                                ...
                                if ((dragDistance >= dragStartDistanceThresholdRelative &&
                                     dragTime >= dragDurationThreshold)
                ...
                if (isPinching == false && isDragging == false && wasPinchingLastFrame == false &&
                    wasDraggingLastFrame == false && isClickPrevented == false)
                {
        ...
        private void UpdatePinch()
        {
            ...
            Vector3 touch0DeltaRelative =
                GetTouchPositionRelative(TouchWrapper.Touches[0].Position - touchPositionLastFrame[0]);
            Vector3 touch1DeltaRelative =
                GetTouchPositionRelative(TouchWrapper.Touches[1].Position - touchPositionLastFrame[1]);
            ...
            if (OnPinchUpdateExtended != null)
            {
                ...
                {
                    pinchCenter = pinchCenter, pinchDistance = pinchDistance, pinchStartDistance = pinchStartDistance,
                    pinchAngleDelta = pinchAngleDelta, pinchAngleDeltaNormalized = pinchAngleDeltaNormalized,
                    pinchTiltDelta = pinchTiltDelta, pinchTotalFingerMovement = totalFingerMovement
                });
            }
